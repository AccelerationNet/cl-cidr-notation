...
(defparameter +max-ip-string-length+ (length "255.255.255.255/32"))
(defparameter +max-range-string-length+
  (+ 1 (* 2 +max-ip-string-length+)))
...
(defun range-string (s0 s1 &aux idx)
  ...
  (let ((s (make-string
            ...
            :initial-element #\.)))
    (setf (values s idx) (%cidr-string s0 :out-string s))
    (setf (char s idx) #\-)
    (incf idx)
    (setf (values s idx)
          (%cidr-string s1 :out-string s :start-idx idx))
    (subseq s 0 idx)))

(defun cidr-string (start &optional (fixed-bits 32))
  (%cidr-string start :fixed-bits fixed-bits))

(defun %cidr-string (start
                    &key
